...
func main() {
	...
	// 出荷データを取得
	shipmentData, err := getShipmentData(db)
	if err != nil {
		log.Fatal("データ取得エラー:", err)
	}

	if len(shipmentData) == 0 {
		fmt.Println("出荷データが見つかりませんでした。")
		return
	}

	// 粗利計算
	reports := calculateProfits(shipmentData)
...
func getShipmentData(db *sql.DB) ([]ShipmentData, error) {
	query := `
SELECT 
	s.target_date,
...
`
	...
	for rows.Next() {
		...
		// Null値の処理
		if salesQty.Valid {
			item.SalesQuantity = int(salesQty.Int32)
		}
		if costQty.Valid {
			item.CostQuantity = int(costQty.Int32)
		}
...
// calculateProfits 粗利を計算
func calculateProfits(data []ShipmentData) []ProfitReport {
	var reports []ProfitReport
...
func printReportRow(report ProfitReport) {
	profitMarginStr := ""
...
// calculateSummary サマリーを計算
func calculateSummary(reports []ProfitReport) Summary {
	var summary Summary
